...
class FaceDetectionModel(LightningModule):
	...
	def on_validation_epoch_start(self):
		self.mAPs = MeanAveragePrecision(box_format="xyxy", class_metrics=False)
	
	# TODO: Q&A
	def on_validation_epoch_end(self) -> None:
		mAP = {"val_" + k: v for k, v in self.mAPs.compute().items()}
		print(mAP)
		self.log_dict(mAP, sync_dist=True, on_epoch=True)
		self.mAPs.reset()
	...
	def eval_step(self, batch, batch_idx, prefix: str):
		...
		self.mAPs.update([preds[i] for i in selected], [targets[i] for i in selected])
